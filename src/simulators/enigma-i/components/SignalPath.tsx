...
const passRotor = (index: number, rotor: { type: string; position: number; ringSetting: number }, forward: boolean): number => {
  ...
  if (forward) {
    ...
    contact = ROTOR_DATA[rotor.type as keyof typeof ROTOR_DATA].wiring.indexOf(targetChar);
  }
...
function traceSignal(char: string, state: MachineState) {
  ...
  steps.push({ label: 'Input', letter: current, color: 'text-stone-200' });

  // Plugboard in
  if (state.plugboard[current]) {
    current = state.plugboard[current];
    steps.push({ label: 'Plug', letter: current, color: 'text-pink-400' });
  } else {
    steps.push({ label: 'Plug', letter: current, color: 'text-stone-500' });
  }

  let signal = toIndex(current);

  // Forward: Right(2) → Mid(1) → Left(0)
  const labels = ['Right', 'Mid', 'Left'];
  for (let i = 2; i >= 0; i--) {
    signal = passRotor(signal, state.rotors[i], true);
    steps.push({ label: labels[2 - i], letter: toChar(signal), color: 'text-yellow-400' });
  }
  ...
  const backLabels = ['Left', 'Mid', 'Right'];
  for (let i = 0; i < 3; i++) {
    signal = passRotor(signal, state.rotors[i], false);
    steps.push({ label: backLabels[i], letter: toChar(signal), color: 'text-yellow-400' });
  }
  ...
  if (state.plugboard[outChar]) {
    ...
    steps.push({ label: 'Plug', letter: outChar, color: 'text-pink-400' });
  } else {
    steps.push({ label: 'Plug', letter: outChar, color: 'text-stone-500' });
  }
  ...
  return (
    <div className="w-full bg-stone-900/80 rounded-xl border border-stone-800 p-5">
      ...
      <div className="mt-3 pt-2 border-t border-stone-800 flex gap-4 text-[10px] font-mono text-stone-600">
        ...
        <span><span className="text-yellow-400">Yellow</span> = Rotor</span>
