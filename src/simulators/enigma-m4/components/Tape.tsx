...
export const Tape: React.FC<TapeProps> = ({ text }) => {
  ...
  // Format text into groups of 4 for authenticity
  ...
  return (
    <div className="w-full max-w-3xl mx-auto my-4 relative">
        ...
        <div 
          ref={containerRef}
          className="bg-[#f0e6d2] text-slate-900 font-typewriter text-xl p-3 overflow-x-auto whitespace-nowrap shadow-inner border-y-4 border-dashed border-slate-800/20 h-16 flex items-center"
          style={{ scrollBehavior: 'smooth' }}
        >
            <span className="opacity-50 select-none mr-4">OUTPUT:</span>
            <span className="tracking-widest font-bold">{formattedText}</span>
            <span className="w-4 inline-block animate-pulse bg-slate-900/50 h-5 ml-1 align-middle"></span>
        </div>
    </div>
  );
};
